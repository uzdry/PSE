...
\subsection{Server-Side Bus}
	Der Bus auf der Serverseite arbeitet synchrone Methodenaufrufe (\code{handleMessage(m: Message): void}) von allen \code{BusDevice} Instanzen ab, die auf das \code{Topic} von \code{m} subscriben. Man kann kontinuierlich Nachrichten auf den Bus legen ohne dass der Server abstürzt. Auf den Referenzgeräten wurden dadurch circa 2/3 der CPU leistung in Anspruch genommen. Die Anzahl der Behandeldeten Nachrichten ist nach wenigen Sekunden außerhalb des durch \code{number} darstellbaren Bereiches.
